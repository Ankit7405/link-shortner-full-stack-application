  ...
  const fetchMyShortUrl = async () => {
    ...
    try {
      const { data } = await api.get(
        `/api/urls/analytics/${selectedUrl}?startDate=2025-01-01T00:00:00&endDate=2025-12-01T23:59:00`,
        {
          headers: {
        ...
        <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden">
          <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2">
            <a
              ...
              target="_blank"
              className=" text-[17px]  font-[montserrat] font-[600] text-[#2a5bd7]"
            >
              {subDomain + "/" + `${shortUrl}`}
            </a>
            ...
            <>
              {analyticsData.length === 0 && (
                <div className="absolute flex flex-col  justify-center sm:items-center items-end  w-full left-0 top-0 bottom-0 right-0 m-auto">
                  <h1 className=" text-slate-800 font-[monstrat] sm:text-2xl text-[15px] font-bold mb-1">
